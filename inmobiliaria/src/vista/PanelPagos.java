...
public class PanelPagos extends JPanel {
    ...
    public PanelPagos(InmobiliariaDB inmobiliaria, PanelContratos panelContratos) {
    ...
    private void cargarDatos() {
        ...
        for (Pago p : inmobiliaria.getPagos()) {
    ...
    private void registrarPago() {
        ...
        btnGuardar.addActionListener(e -> {
            try {
                ...
                Pago pago = new Pago(0, idContrato, fecha, monto, metodo, observaciones);
                inmobiliaria.agregarPago(pago);
    ...
    private void generarReciboParaPago(Pago pago) {
        ...
        if (formato >= 0) {
            try {
                ...
                Recibo recibo = new Recibo(0, pago.getIdPago(), LocalDate.now(), opciones[formato]);
                inmobiliaria.agregarRecibo(recibo);
                ...
                JOptionPane.showMessageDialog(this,
                        "Error al guardar el recibo: " + ex.getMessage(),
                        "Error", JOptionPane.ERROR_MESSAGE);
            }
        }
    }
    ...
    private boolean tieneRecibo(int idPago) {
        return inmobiliaria.existeReciboPorPago(idPago);
    }

    private Pago buscarPagoPorId(int id) {
        return inmobiliaria.buscarPagoPorId(id);
    }

    private Recibo buscarReciboPorIdPago(int idPago) {
        return inmobiliaria.buscarReciboPorIdPago(idPago);
    }
