...
- **Java Collections**: ArrayList para manejo de datos en memoria
...
│   ├── dao/
│   │   └── ConexionDB.java                # Manejo de conexión a MySQL
│   └── excepciones/
...
├── ejecutar_proyecto.sh                   # Script completo (MySQL + compilar + ejecutar)
├── ejecutar_simple.sh                     # Script rápido (asume DB creada)
├── probar_conexion.sh                     # Diagnóstico de conexión a MySQL
├── descargar_mysql_connector.sh           # Descarga e instala MySQL Connector/J
└── README.md
```
...
- Usuario y base de datos configurados (por defecto):
  - Base de datos: `inmobiliaria_db`
  - Usuario: `inmobiliaria`
  - Contraseña: `inmobiliaria123`

Estos valores se pueden cambiar en `src/dao/ConexionDB.java`.

### 2. Primer uso (entorno nuevo)
...
- Verifica/descarga `mysql-connector-j.jar` en `lib/`
- Opcionalmente verifica/crea la base de datos `inmobiliaria_db`
- Compila todo el proyecto con el driver de MySQL en el classpath
- Ejecuta la aplicación principal

### 3. Ejecución rápida (cuando ya está todo configurado)
...
- La base de datos `inmobiliaria_db` ya existe
- Las credenciales en `ConexionDB.java` son correctas

### 4. Probar solo la conexión a MySQL
...
Compila y ejecuta una clase de prueba que verifica la conexión definida en `ConexionDB.java`.
...
- El sistema utiliza `ArrayList` para manejo interno de datos en memoria
- La persistencia de la información principal se realiza en **MySQL** mediante `dao/ConexionDB.java`
